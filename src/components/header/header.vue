<template>
    <div class="header">
        <img src="data:image/png;base64,"
            alt="">
        <p class="title">学习面面</p>
        <div class="isfold" @click="changeFold">
            <i class="iconfont icon-zhankaicaidan"></i>
        </div>
        <div>
            <el-breadcrumb separator="/" >
                <el-breadcrumb-item :to="{ path: '/dashboard' }" v-if="path=='/dashboard'">数据概览</el-breadcrumb-item>
                <template v-else>
                <el-breadcrumb-item :to="{ path: '/dashboard' }">面板</el-breadcrumb-item>
                <el-breadcrumb-item>{{ firstTitle }}</el-breadcrumb-item>
                <el-breadcrumb-item>{{ title}}</el-breadcrumb-item>
                </template>
            </el-breadcrumb>
        </div>
        <div class="exit">
            <el-dropdown>
      <el-button type="primary">
        Demo
        <i class="iconfont icon-a-xiala2"></i>
      </el-button>
      <template #dropdown>
        <el-dropdown-menu>
          <el-dropdown-item @click="goDashboard">首页</el-dropdown-item>
          <el-dropdown-item @click="exitLogin">退出登录</el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
        </div>
    </div>
</template>

<script lang="ts" setup>
import emmitter from '@/utils/mitt';
import { useRouter } from 'vue-router';
import {watch,reactive,toRefs} from 'vue'

const changeFold = () => {
    emmitter.emit('isCollapse')
}
const router = useRouter()

let state = reactive<{firstTitle:string;title:string;path:string}>({
    firstTitle:'',
    title:'',
    path:''
})
const {firstTitle,title,path} = toRefs(state)

const emit = defineEmits<{
    (event: 'record', content: RecordTtf): void
}>()
watch(()=>router.currentRoute.value,(a)=>{
    firstTitle.value=a.matched[0].meta.title as string
    title.value=a.meta.title as string
    path.value=a.path
    if(path.value!=='/login'){
        emit('record',{title:title.value,path:path.value})
    }
})
const goDashboard=()=>{
    router.push('/dashboard')
}
const exitLogin=()=>{
    window.localStorage.removeItem('token')
    router.push('/login')
}

</script>

<style>
.header {
    width: 100%;
    height: 70px;
    background: #1493fa;
    display: flex;
    align-items: center;
    position: sticky;
    top: 0;
    z-index: 999;
}

.header img {
    margin-left: 20px;
}

.title {
    font-size: 22px;
    margin-left: 10px;
    color: white;
}

.exit {
    position: absolute;
    right: 20px;
}

.isfold {
    color: white;
    margin:  0 20px;
    cursor: pointer;
}

.isfold .iconfont {
    font-size: 20px;
}
.el-breadcrumb__inner,.el-breadcrumb__item:last-child .el-breadcrumb__inner{
    color: #ffffff !important;
}
</style>
